...
5. テストの実行
   ...
   - テスト項目の「4. 日時取得機能」と、「16-2. "認証はこちらから"ボタンを押下するとメール認証サイトに遷移する」は、
     下記のコマンドにて実行します。  
     ※JavaScript で表示している時間表示のテストが、数秒程度のずれによってエラーとなる場合があるようです。
     もしエラーが起きた場合は、再度実行してみてください。
     ```
     $ php artisan dusk
     ```
...
1. ユーザー
   ...
   - ランダムな仮名とメールアドレスの一般職員 3 名（パスワードは password ）

2. 勤怠に関する各種データ

   - 職員の勤怠情報（9 時間以上勤務している従業員の合計勤務時間は赤字で表記されています）
